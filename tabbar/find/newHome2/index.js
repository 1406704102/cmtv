...
Page({
	...
	onChange(e) {
		let that = this;
		console.log('change----------------------------------', e.detail.dataIdx);
		console.log('change----------------------------------', 3 * (that.data.page + 1) -2);
		if (3 * (that.data.page + 1) -2 === e.detail.dataIdx) {
			wx.getStorage({
				key: 'userInfo',
				success(res) {
					wx.request({
	...
	onPanelForward(e) {
		// console.log("videoInfo",e.detail.video.videoInfo)
		this.setData({
			videoInfo:e.detail.video.videoInfo
		})
	...
	onShareTimeline() {
		const that = this;
		return {
			title: that.data.videoInfo.videoSynopsis,
			query: 'timeline=1&share=share&videoId=' + that.data.videoInfo.id
		}
	},
