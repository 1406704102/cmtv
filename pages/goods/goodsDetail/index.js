...
Page({
  ...
  onShareAppMessage: function() {
    ...
    return {
      ...
      imageUrl: that.data.goods.picUrl,
      path: 'pages/goods/goodsDetail/index?id=' + this.data.id +'&share=1'
    }
  },
  onShareTimeline() {
    const that = this;
    return {
      title: that.data.goods.name,
      imageUrl: that.data.goods.picUrl,
      query:'id=' + this.data.id + '&share=1&timeline=1'
    }
  },
    ...
    util.request(api.GoodsDetail, {
      ...
      if (res.errno === 0) {
        ...
        that.setData({
          ...
          checkedSpecCmb: res.data.productList[0].cmb,
    ...
    if (this.isCheckedAllSpec()) {
      ...
      if (checkedProduct.number > 0) {
        this.setData({
          ...
          checkedSpecCmb: checkedProduct.cmb,
          soldout: false
        });
      } else {
        this.setData({
  ...
  onShow: function() {
    ...
    wx.getStorage({
      ...
      ,fail(res) {
        wx.navigateTo({
          url: '/pages/my/login/login?type=0&id=' + that.data.goods.id
        })
      }
    })
  },
  ...
  addFast: function() {
    ...
    if (this.data.openAttr == false) {
      ...
      //如果商品是优惠卷 需要半段 是否 已经买过
      if ("coupontype" in that.data.goods) {
        wx.getStorage({
          key: 'userInfo',
          success(res) {
            wx.request({
              ...
              success(res) {
                ...
                let coupon = that.data.goods.coupontype + ',';
                ...
                if (user.coupons === null) {
                      ...
                      .then(function (res) {
                        if (res.errno == 0) {
                          ...
                          try {
                            ...
                            wx.navigateTo({
                              url: '/pages/goods/checkout/checkout?coupontype=' + that.data.goods.coupontype
                            })
                          } catch (e) {
                          }
                  ...
                  let index = user.coupons.indexOf(coupon);
                  let num = 0;
                  while (index !== -1) {
                    num++;
                    index = user.coupons.indexOf(coupon, index + 1);
                  }
                  var strings = coupon.split("-");
