      ...
      Component({
        ...
        methods: {
          _videoListChanged: function _videoListChanged(newVal) {
            ...
            // console.log("newVal.size", data.nextQueue.length);
            // console.log("data", data)
            if (data.curQueue.length === 0) {
              // console.log("data.nextQueue.splice(0, 3)",data.nextQueue.splice(0, 3))
              this.setData({
                _allList: data.nextQueue.length,
                curQueue: data.nextQueue.splice(0, 3)
              }, function () {
                _this.playCurrent(1);
              });
            }
            console.log(_this.data._allList);
          },
          animationfinish: function animationfinish(e) {
            var _data = this.data,
              _allList = _data._allList,
              _last = _data._last,
              _nowShow = _data._nowShow,
              _change = _data._change,
              curQueue = _data.curQueue,
              prevQueue = _data.prevQueue,
              nextQueue = _data.nextQueue;

            // console.log("_data.curQueue", curQueue);

            var current = e.detail.current;
            // console.log("current", current);
            // console.log("_last", this.data._last);

            var diff = current - _last;
            // console.log("diff-------------------", diff)
            if (diff === 0) return;
            // console.log(_nowShow === _allList)
            if (this.data._nowShow === this.data._allList) return false;
            this.data._last = current;
            this.playCurrent(current);
            this.triggerEvent('change', {
              activeId: curQueue[current].id
            });
            // console.log("activityId", curQueue[current].id)
            var direction = diff === 1 || diff === -2 ? 'up' : 'down';

            // console.log("_nowShow", _nowShow);
            // console.log("_allList", _allList);
            if (this.data._nowShow !== this.data._allList) {
              if (direction === 'up') {
                if (this.data._invalidDown === 0) {
                  ...
                  if (add) {
                    ...
                    // console.log("add",add)
                    curQueue[change] = add;
                    // console.log("_data.curQueue-new", curQueue);
          ...
          playVideo: function playVideo(e) {
            if (this.data._isPlaying) {
              this.pauseCurrent(e.currentTarget.dataset.index)
              this.data._isPlaying = false
              this.trigger(e, "showPause")
            } else {
              this.playCurrent(e.currentTarget.dataset.index)
              this.data._isPlaying = true
              this.trigger(e, "hidePause")
            }
          },
